...
function createBrowserView() {
    ...
    const emitUrlChanged = (u) => {
        if (mainWindow && !mainWindow.isDestroyed()) {
            mainWindow.webContents.send('browser:urlChanged', u);
        }
    };
    browserView.webContents.on('will-navigate', (_event, u) => emitUrlChanged(u));
    browserView.webContents.on('did-navigate', (_event, u) => emitUrlChanged(u));
    browserView.webContents.on('did-navigate-in-page', (_event, u) => emitUrlChanged(u));
    ...
    browserView.webContents.on('context-menu', () => {
        if (mainWindow && !mainWindow.isDestroyed()) {
            const menu = electron_1.Menu.buildFromTemplate(contextMenuTemplate);
    ...
    browserView.webContents.on('console-message', (_event, _level, message, _line, sourceId) => {
        const level = _level;
        if (level >= 4)
            console.error('[browserView]', message, sourceId ? `(${sourceId})` : '');
    });
    ...
    browserView.webContents.on('did-finish-load', async () => {
        if (!gate7Active || gate7StoreId == null || !browserView || !mainWindow?.webContents || mainWindow.isDestroyed())
            return;
...
async function captureListingSnapshotFromWebContents(wc, storeId) {
    const url = wc.getURL();
    if (!url || !url.includes('/listing/'))
        return null;
    ...
    (0, db_1.insertListingSnapshot)(id, storeId, 'seo_audit', url, parsed.title ?? null, parsed.description ?? null, tagsJson, imageCount);
    return {
        id,
        listing_url: url,
...
function createWindow() {
    ...
    mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
        ...
        if (!electron_1.app.isPackaged && isMainFrame && validatedURL && validatedURL.includes('localhost:5173')) {
            if (mainWindow && !mainWindow.isDestroyed()) {
                const html = `
        <!DOCTYPE html>
        <html>
...
function registerIpcHandlers() {
    ...
    electron_1.ipcMain.on('gate7:closeBrowser', () => {
    ...
    electron_1.ipcMain.handle('gate7:setContext', (_event, payload) => {
        gate7StoreId = payload?.storeId ?? null;
    });
    electron_1.ipcMain.on('browser:setBounds', (_event, rect) => {
        if (!browserView)
            return;
    ...
    electron_1.ipcMain.handle('nav:go', (_event, url) => {
        if (!browserView)
            return;
        let u = url.trim();
    ...
    electron_1.ipcMain.handle('getCurrentUrl', () => {
        if (!browserView)
            return '';
        return browserView.webContents.getURL();
    });
    electron_1.ipcMain.handle('session:create', (_event, id, note) => {
        if (!(0, db_1.getSession)(id))
            (0, db_1.insertSession)(id, note);
        (0, db_1.setSessionCompetitorUrl)(id, null);
    });
    electron_1.ipcMain.handle('competitor:set', (_event, payload) => {
        const { sessionId, url } = payload;
        if (!sessionId || !url)
            return;
        (0, db_1.setSessionCompetitorUrl)(sessionId, url.trim());
    });
    electron_1.ipcMain.handle('competitor:clear', (_event, sessionId) => {
        if (!sessionId)
            return;
        (0, db_1.setSessionCompetitorUrl)(sessionId, null);
    });
    electron_1.ipcMain.handle('getCompetitorUrl', (_event, sessionId) => {
        if (!sessionId)
            return null;
        return (0, db_1.getSessionCompetitorUrl)(sessionId);
    });
    electron_1.ipcMain.handle('getLatestCompetitorSignals', (_event, sessionId) => {
        if (!sessionId)
            return null;
        try {
            const row = (0, db_1.getLatestCompetitorCapture)(sessionId);
            if (!row || !row.signals_json)
                return null;
    ...
    electron_1.ipcMain.handle('gate7:captureListing', async (_e, payload) => {
    ...
    electron_1.ipcMain.handle('ping', async (_e, p) => {
        console.log('PING_HANDLER', p);
        return { ok: true, pong: true };
    });
    electron_1.ipcMain.handle('competitor:capture', async (_e, payload) => {
        ...
        try {
            ...
            const pngPath = path_1.default.join(assetsDir, id + '.png');
            try {
            ...
            try {
                (0, db_1.insertCompetitorCapture)(id, sessionId, url, pngPath, '{}');
            ...
            let signals = { title: null, price: null, photoCount: null, rating: null, reviewCount: null, shippingCue: null };
            try {
        ...
        `);
            ...
            try {
                (0, db_1.updateCompetitorSignals)(id, signalsJson);
            }
            catch (e) {
    ...
    electron_1.ipcMain.handle('capture:create', async (_event, payload) => {
        ...
        try {
            ...
            if (!(0, db_1.getSession)(providedSessionId))
                (0, db_1.insertSession)(providedSessionId);
            ...
            const htmlPath = path_1.default.join(assetsDir, captureId + '.html');
            const pngPath = path_1.default.join(assetsDir, captureId + '.png');
            let html;
            try {
            ...
            try {
                ...
                fs_1.default.writeFileSync(pngPath, png);
            }
            catch (e) {
            ...
            try {
                (0, db_1.insertCapture)(captureId, providedSessionId, url, htmlPath, pngPath, parseStatus, parsedJson);
    ...
    electron_1.ipcMain.handle('listSessions', () => (0, db_1.listSessions)());
    electron_1.ipcMain.handle('listStores', () => (0, db_1.listStores)());
    electron_1.ipcMain.handle('updateStoreGoal', (_event, id, goal) => {
        (0, db_1.updateStoreGoal)(id, goal ?? null);
        return { ok: true };
    });
    electron_1.ipcMain.handle('getSession', (_event, id) => (0, db_1.getSession)(id));
    electron_1.ipcMain.handle('getSetting', (_event, key) => (0, db_1.getSetting)(key));
    electron_1.ipcMain.handle('setSetting', (_event, key, value) => (0, db_1.setSetting)(key, value));
    electron_1.ipcMain.handle('updateSessionNote', (_event, sessionId, note) => {
        (0, db_1.insertSession)(sessionId);
        (0, db_1.updateSessionNote)(sessionId, note);
    });
    electron_1.ipcMain.handle('getCapture', (_event, id) => (0, db_1.getCapture)(id));
    electron_1.ipcMain.handle('getAiOutput', (_event, captureId, type) => {
        const row = (0, db_1.getAiOutputByCapture)(captureId, type);
        if (!row)
            return null;
        try {
    ...
    electron_1.ipcMain.handle('listCaptures', (_event, sessionId) => {
        const rows = (0, db_1.listCapturesBySession)(sessionId);
        console.log('[main] listCaptures', { sessionId, count: rows.length, ts: Date.now() });
        return rows;
    });
    electron_1.ipcMain.handle('getCaptureImage', (_event, captureId) => {
        const capture = (0, db_1.getCapture)(captureId);
        if (!capture)
            return null;
        try {
            const buf = fs_1.default.readFileSync(capture.png_path);
    ...
    electron_1.ipcMain.handle('getParsedListing', (_event, captureId) => {
        const capture = (0, db_1.getCapture)(captureId);
        if (!capture)
            return null;
        try {
            const html = fs_1.default.readFileSync(capture.html_path, 'utf-8');
            return (0, parser_1.parseListing)(html, capture.url);
        }
        catch {
            return null;
        }
    });
    electron_1.ipcMain.handle('capture:analyze', async (_event, captureId) => {
        try {
            const capture = (0, db_1.getCapture)(captureId);
            if (!capture) {
            ...
            let html;
            try {
                html = fs_1.default.readFileSync(capture.html_path, 'utf-8');
            }
            catch {
                return { ok: false, errorMessage: 'Failed to read capture data.' };
            }
            const apiKey = (0, db_1.getSetting)('openai_api_key') || process.env.OPENAI_API_KEY;
            if (!apiKey) {
                return { ok: false, errorMessage: 'OpenAI API key not set. Add it in Settings.' };
            }
            let result;
            try {
                result = await (0, openai_1.runSeoAudit)(html, capture.url);
            }
            catch (e) {
                const msg = e instanceof Error ? e.message : String(e);
            ...
            try {
                ...
                (0, db_1.insertAiOutput)(outputId, captureId, 'seo_audit', JSON.stringify(result));
            }
            catch {
...
electron_1.app.whenReady().then(async () => {
    ...
    electron_1.Menu.setApplicationMenu(electron_1.Menu.buildFromTemplate(editMenuTemplate));
    registerIpcHandlers();
});
